...
export const MyLikedPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (response.success) {
          // 이미지 URL을 절대 경로로 변환
          const productsWithFullUrls = response.posts.map((post: LikedProduct) => ({
            ...post,
  ...
  const sortProducts = (products: LikedProduct[], sort: string): LikedProduct[] => {
    ...
    <Layout>
      <div className="max-w-content mx-auto px-4 md:px-8 lg:px-20 py-8">
        ...
        <div className="mb-6">
          ...
          <h2 className="text-xl md:text-2xl font-bold text-gray-900">관심목록</h2>
          ...
          <>
            {paginatedProducts.length === 0 ? (
              <div className="flex flex-col justify-center items-center py-20">
                ...
                <p className="text-gray-500 text-base md:text-lg">관심 상품이 없습니다.</p>
              </div>
            ) : (
              <>
                {/* 모바일: 리스트형 */}
